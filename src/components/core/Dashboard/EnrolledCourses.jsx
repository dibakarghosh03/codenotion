...
export default function EnrolledCourses() {
    ...
    return (
        ...
        {!enrolledCourses ? (
            ...
            <>
                <div className="hidden sm:block my-8 text-richblack-5">
                    {/* Headings */}
                    ...
                    {enrolledCourses.map((course, i, arr) => (
                        ...
                        <div className="flex w-fit flex-col  gap-2 px-2 py-3">
                ...
                <div className="block sm:hidden">
                    {enrolledCourses.map((course, i) => (
                        <div className="max-w-[340px] mx-auto px-2 border rounded-lg my-10 py-4 border-richblack-400">
                            <div onClick={() => {navigate(`/view-course/${course?._id}/section/${course.courseContent?.[0]?._id}/sub-section/${course.courseContent?.[0]?.subSection?.[0]?._id}`)}} >
                                <div className="flex justify-center">
                                    <img
                                        src={course.thumbnail}
                                        alt="course_img"
                                        className="rounded-lg max-h-44 object-cover cursor-pointer"
                                    />
                                </div>
                                <div className="my-4">
                                    <p className="text-white">{course.courseName}</p>
                                    <p className="text-pureGreys-200">
                                        {course.courseDescription.length > 50
                                        ? `${course.courseDescription.slice(0, 50)}...`
                                        : course.courseDescription}
                                    </p>
                            ...
                            <div className="text-caribbeangreen-50">
                                <ProgressBar
                                completed={course.progressPercentage || 0}
                                height="8px"
                                isLabelVisible={false}
